...
npcw_width, npcw_height, npcw_x, npcw_y = (215, 100, 550, 200) # NameOrPasChangeWindow geometry
...
class popupWindow:
    ...
    def __init__(self, master):
        ...
        icon1 = tk.PhotoImage(file = os.path.join(PATH, 'img', 'icon3.png')) # Icon
        top.tk.call('wm', 'iconphoto', top._w, icon1)
        top.focus_force()
        self.l = tk.Label(top, text=' Password: ', font=('Bitstream Vera Serif', 15), justify=tk.CENTER)
        self.l.pack(pady=3)
        self.e = tk.Entry(top, show='*', width=25, bd=3)
        self.e.pack(pady=7)
        self.e.bind('<Return>', self.cleanup)
        self.b = tk.Button(top, text='Submit', command=self.cleanup, font=main_font, bd=2)
        self.b.pack(pady=3)
        self.e.focus()
        top.protocol('WM_DELETE_WINDOW', self.on_closing) # do after close button pressed
        ...
        if self.value:
            if self.value == access:
                ...
                if self.attempts == 3:
                    root.quit()
                self.e .delete(0, 'end')
                messagebox.showerror('Incorrect Password', 'Wrong password!\nRemaining attempts: ' + str(3 - self.attempts))
...
class addWindow:
    ...
    def __init__(self, master):
        ...
        icon2 = tk.PhotoImage(file = os.path.join(PATH, 'img', 'icon3.png')) # Icon
        window.tk.call('wm', 'iconphoto', window._w, icon2)
        window.focus_force()

        self.head_label = tk.Label(window, text='New Account', font=('Bitstream Vera Serif', 18))
        ...
        self.password.bind('<Return>', self.addacc_command) # Binding Enter to do same as add button
        self.generate_b = tk.Button(window, text='Generate', command=self.generate_command, font=main_font, bg=GENERATE_BTN_COLOR, bd=3, padx=10, pady=8)
        self.generate_b.config(highlightbackground=HIGHLIGHT_COLOR)
        self.submit_b = tk.Button(window, text='Add', command=self.addacc_command, font=main_font, bg='white', bd=3, padx=40, pady=8)
        ...
        window.protocol('WM_DELETE_WINDOW', self.on_closing) # do after close button pressed
...
class NameOrPasChangeWindow:
    ...
    def __init__(self, master, oldname=''):
        ...
        popWin.geometry('{}x{}+{}+{}'.format(npcw_width, npcw_height, npcw_x, npcw_y))
        popWin.resizable(width=False, height=False)
        icon3 = tk.PhotoImage(file = os.path.join(PATH, 'img', 'icon3.png')) # Icon
        ...
        self.h_label = tk.Label(popWin, text = f'Enter new {what}', font=main_font)
        self.h_label.pack()
        self.en_box = tk.Entry(popWin, width=25, bd=3, font=main_font)
        if oldname:
            self.en_box.insert(0, oldname)
        self.en_box.pack(padx=5)
        self.en_box.bind('<Return>', self.adname) # binding Enter to same Button function
        self.en_box.focus()
        ...
        popWin.protocol('WM_DELETE_WINDOW', self.on_closing) # do after close button pressed
    ...
    def adname(self, event=None):
        ...
        if enteredname:
            ...
            os.rename(pather2file, os.path.join(DB_PATH, enteredname + 'Acc.txt'))
            ok = messagebox.showinfo('Success', f'{self.oldname} was successfuly changed to {enteredname}')
            if ok == 'ok':
                self.on_closing()
...
def delete_acc():
    tup = listbox.curselection()
    if tup:
        answer = messagebox.askquestion('Delete', 'Are you sure you want to delete this account?')
        if answer == 'yes':
            currentselection = tup[0]
            filepth = os.path.join(DB_PATH, listbox.get(currentselection) + 'Acc.txt')
            os.remove(filepth)
            update_listbox()
            status_bar.config(text='')
...
option_menu.add_command(label='Change Font') # changes whole status, infolabel and midframe colors
...
listbox = tk.Listbox(top_frame, width=40, height=15, font=listbox_item_font, yscrollcommand=scrollbar.set)
...
# Middle Frame
del_button = tk.Button(mid_frame, text='Delete', font=button_font, bg=DEL_BTN_COLOR, fg='white', bd=3, command=delete_acc)
del_button.pack(side='left', ipadx=15, ipady=7, padx=(1,4))
show_button = tk.Button(mid_frame, text='Show', font=button_font, bd=3, bg=SHOW_BTN_COLOR, fg='white', command=show_info)
show_button.pack(side='left', ipadx=15, ipady=7)
add_button = tk.Button(mid_frame, text='Add Account', font=button_font, bg=ADD_BTN_COLOR, bd=3, command=lambda: addWindow(root))
add_button.config(highlightbackground=HIGHLIGHT_COLOR)
add_button.pack(side='left', ipadx=15, ipady=7, padx=(31,1))

# Bottom Frame
info_label = tk.Label(bot_frame, width=50, height=6, justify=tk.LEFT, bg=MAIN_COLOR, font=('Arial', 16))
info_label.pack()
